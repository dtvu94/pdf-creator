...
import { registerFontsClient, getDefaultFamily } from "@/lib/fontRegistry";
...
/** Absolutely-positioned table — used for body elements on pages WITHOUT header/footer sections. */
      ...
      {el.rows.map((row, rowIdx) => (
        ...
        >
          {row.map((cell, cellIdx) => (
            <View
              key={`pdf-table-element-row-${rowIdx + 1}-cell${cellIdx + 1}`}
              style={{
                width: colWidth,
                padding: "4 6",
                borderRight:
                  cellIdx < row.length - 1 ? "1 solid #E2E8F0" : "none",
              }}
            >
              <Text style={{ fontSize: el.fontSize, color: "#374151" }}>
                {cell}
              </Text>
            </View>
          ))}
...
/**
 * Flow-positioned table — used for body elements on pages WITH header/footer sections.
 * Not absolutely positioned so react-pdf can insert page breaks between rows,
 * while the fixed header/footer Views repeat on every overflow page.
 */
function PdfTableFlowElement({ el }: Readonly<{ el: TableElement }>) {
  ...
  return (
    <View style={{ marginLeft: el.x, marginTop: el.y, width: el.width }}>
      ...
      {/* Data rows — wrap={false} keeps each row on one page */}
      {el.rows.map((row, rowIdx) => (
        <View
...
function PdfChartElement({ el }: Readonly<{ el: ChartElement }>) {
  ...
  // Fallback placeholder when no image has been rendered (e.g. server-side without pre-render).
  return (
    <View
      style={{
        position: "absolute",
...
function PdfShapeElement({ el }: Readonly<{ el: ShapeElement }>) {
  ...
  // rectangle / circle — use View with border-radius
...
function substituteItem(el: TemplateElement, item: RepeaterItem): TemplateElement {
  const { fields, chartImages = {} } = item;
  const sub = (t: string) =>
    t.replaceAll(ITEM_PH_RE, (_, k: string) => fields[k.trim()] ?? `{{${k}}}`);
  ...
  if (el.type === "table") {
    ...
    // If rowsDataField is set, pull rows from item data (JSON array of arrays).
    if (t.rowsDataField && fields[t.rowsDataField]) {
      ...
      } catch { /* keep existing rows if parse fails */ }
    }
  ...
  if (el.type === "image") {
    ...
    // If srcField is set, pull image src from item data.
          ...
          {row.map((item, colIdx) => (
            ...
            >
              {el.cardElements.map((cEl) => {
                const resolved = substituteItem(cEl, item);
...
/** Renders a single page, choosing the layout mode based on whether it has header/footer sections or flow elements. */
...
function PdfPage({
  ...
  const hasFlowElements = page.elements.some(
    (el) => (el.type === "table" && el.mode === "auto") || el.type === "repeater"
  );
  ...
  return (
    /*
     * paddingTop/paddingBottom on the Page reserves those bands on EVERY overflow page,
     * so the body flow never enters the header or footer zones.
     * Absolutely-positioned Views (header/footer) ignore Page padding and anchor to
     * the true page edges, so they still render in the correct position.
     */
    <Page
      size={pageSize as "A4"}
      style={{
    ...
    >
      {useFlowLayout ? (
        <>
          ...
          {/* Fixed footer — repeats on every overflow page */}
          {page.footer && (
            <View
              fixed
              style={{
          ...
          {/* Body — Page padding already reserves header/footer space on every page */}
          <View>
            {page.elements.map((el) =>
              el.type === "table" && el.mode === "auto" ? (
        ...
        // Legacy absolute layout for pages without sections or flow elements
...
export function PdfDocument({ template, metadata }: Readonly<PdfDocumentProps>) {
  // Register fonts before the document tree is evaluated by react-pdf.
  registerFontsClient(template.fonts);
